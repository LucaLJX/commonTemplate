<template>
  <div>
    <filterBar :type="type" @change-filter="changeHandler" />
    <div class="division-row"></div>
    <ul class="course-list" v-infinite-scroll="requestData" infinite-scroll-disabled="loading" infinite-scroll-distance="10">
      <courseCard :key="item.id" :course="item" v-for="item in list">
        <router-link class="btn btn-primary" slot="btn" :to="`/course/${item.id}`">试听</router-link>
      </courseCard>
    </ul>
    <p v-show="loading" class="infinite-loading">{{ msg }}</p>
  </div>
</template>

<script>
import filterBar from './filter-bar'
export default {
  data () {
    return {
      loading: false,
      msg: '加载中...',
      type: parseInt(this.$route.query.id, 10) || 51,
      code: null,
      sort: 1,
      pageNo: 1,
      list: []
    }
  },
  methods: {
    requestData () {
      this.loading = true
      this.$axios.post('/ace/course/flip', {
        type: this.type,
        moduleCode: this.code,
        sortType: this.sort,
        pageNo: this.pageNo
      }).then(data => {
        if (!data.listInfo || !data.listInfo.length) {
          this.msg = '没有更多了'
          return
        }
        this.pageNo++
        this.loading = false
        this.list = [...this.list, ...data.listInfo]
      }).catch(e => {})
    },
    changeHandler (type, code, sort) {
      this.msg = '加载中...'
      this.type = type
      this.code = code
      this.sort = sort
      this.pageNo = 1
      this.list = []
      this.requestData()
    }
  },
  components: {
    filterBar
  }
}
</script>

<style lang="less" scoped>
@import '../lib.less';
.division-row {
  margin-top: 44px;
  &:before {
    display: none;
  }
}
.course-card {
  .border-bottom;
  .btn {
    width: 60px;
    height: 25px;
    line-height: 25px;
  }
}
</style>
